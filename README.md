...
| 模组数量 | 组合总数       | CPU 版本耗时 | CPU 每秒处理 | GPU 版本耗时 | GPU 每秒处理 | 性能提升 |
| -------- | -------------- | ------------ | ------------ | ------------ | ------------ | -------- |
| 100      | 3,921,225      | 0.192s       | ~20.4M       | 0.142s       | ~27.6M       | 1.35x    |
| 200      | 64,684,950     | 3.772s       | ~17.1M       | 0.144s       | ~449.2M      | 26.2x    |
| 300      | 330,791,175    | 19.546s      | ~16.9M       | 0.829s       | ~399.0M      | 23.6x    |
| 400      | 1,050,739,900  | 74.437s      | ~14.1M       | 3.160s       | ~332.5M      | 23.6x    |
| 500      | 2,573,031,125  | 222.316s     | ~11.6M       | 8.603s       | ~299.1M      | 25.8x    |
| 800      | 10,668,000,000 | -            | -            | 84.985s      | ~125.5M      | -        |
| 1000     | 41,417,124,375 | -            | -            | 252.575s     | ~164.0M      | -        |
...
> ⚠️ **枚举模式限制**: 枚举模式最多支持 500 个模组的运算，如果模组数量超过 500 个，程序会自动筛选至 500 个进行计算(GPU 模式为 800)
...
4. **枚举模式限制**: 枚举模式最多支持 500 个模组的运算，超过后会自动筛选至 500 个(GPU 模式为 800)
...
- **CUDA Toolkit 12.6**
- 支持 CUDA 的 NVIDIA 显卡（GTX 1050 及以上）
